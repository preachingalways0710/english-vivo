...
add_shortcode('english_vivo', function () {
    ...
    <div class="app">
      ...
      <nav>
        <button class="nav-btn active" id="nav-plano" onclick="showSection('plano')">O Plano</button>
        <button class="nav-btn" id="nav-explorar" onclick="showSection('explorar')">Explorar</button>
      </nav>
      ...
      <div id="plano" class="section active">
        <div id="plano-no-exam" class="no-exam-banner" style="display:none">
          ...
          <div style="font-size:16px;font-weight:500;margin-bottom:8px">Comece pelo exame</div>
          <p style="font-size:14px;color:var(--color-text-secondary);margin-bottom:1.25rem">O plano é gerado com base no seu nível. Faça o exame de nivelamento primeiro.</p>
          <button class="btn btn-primary" onclick="showExam()">Fazer o exame →</button>
        </div>
        <div id="plano-generating" style="display:none;text-align:center;padding:3rem 1rem">
          ...
          <div style="font-size:15px;font-weight:500;margin-bottom:6px">Gerando seu plano...</div>
          <div style="font-size:13px;color:var(--color-text-secondary)">Criando milestones para o nível <span id="gen-level"></span></div>
        </div>
        ...
        <div id="exam-panel" style="display:none">
          <div id="exam-intro-panel">
            <button class="btn btn-sm" onclick="hideExam()" style="margin-bottom:1rem;display:none" id="exam-back-btn">← Voltar</button>
            <h1>Exame de Nivelamento</h1>
            <p class="subtitle">Três fases · ~10 minutos · A IA determina seu nível</p>
            <div class="card">
              <p style="font-size:15px;line-height:1.7;margin-bottom:1rem">Este exame avalia seu inglês em leitura, escrita livre e gramática. Ao final você recebe um nível CEFR (A1 a C2) e seu plano de estudos é gerado automaticamente.</p>
              <button class="btn btn-primary" onclick="startExam()">Começar →</button>
            </div>
            <div id="exam-prev" style="display:none">
              <div class="card" style="border:2px solid var(--color-border-info)">
                <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:4px">Último resultado</div>
                ...
                <button class="btn btn-sm" style="margin-top:1rem" onclick="startExam()">Refazer exame</button>
              </div>
            </div>
          </div>
          ...
          <div id="exam-grading-panel" style="display:none;text-align:center;padding:3rem 1rem">
            ...
            <div style="font-size:16px;font-weight:500;margin-bottom:8px">Avaliando suas respostas...</div>
          </div>
          <div id="exam-result-panel" style="display:none">
            <div class="card" style="margin-bottom:1rem;border:2px solid var(--color-border-info)">
              <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:4px">Seu nível CEFR</div>
            ...
            <button class="btn btn-primary" onclick="afterExam()">Ver meu plano →</button>
          </div>
        </div>
      </div>
      ...
      <div id="explorar" class="section">
        <div id="exp-home">
          ...
          <div class="exp-grid">
            <div class="exp-tile" onclick="expShow('stories')"><div class="exp-tile-icon">📖</div><div class="exp-tile-name">Histórias</div><div class="exp-tile-desc">Leitura em inglês com tradução</div></div>
            <div class="exp-tile" onclick="expShow('debate')"><div class="exp-tile-icon">⚔️</div><div class="exp-tile-name">Debate</div><div class="exp-tile-desc">A IA joga contra você (em inglês)</div></div>
            <div class="exp-tile" onclick="expShow('topics')"><div class="exp-tile-icon">🔍</div><div class="exp-tile-name">Gramática</div><div class="exp-tile-desc">Pergunte o que quiser</div></div>
            <div class="exp-tile" onclick="expShow('vocab')"><div class="exp-tile-icon">📚</div><div class="exp-tile-name">Vocabulário</div><div class="exp-tile-desc">Palavras salvas</div></div>
          </div>
        </div>
        <div id="exp-stories" style="display:none">
          <button class="btn btn-sm" onclick="expBack()" style="margin-bottom:1rem">← Explorar</button>
          <div id="exp-story-list">
            <div class="card card-btn" onclick="expOpenStory(0)"><span class="tag tag-green">Igreja</span><div style="font-size:15px;font-weight:500">The Sunday visitor</div><div style="font-size:13px;color:var(--color-text-secondary)">Um missionário americano visita a IBBV</div></div>
            <div class="card card-btn" onclick="expOpenStory(1)"><span class="tag tag-amber">Cotidiano</span><div style="font-size:15px;font-weight:500">At the grocery store</div><div style="font-size:13px;color:var(--color-text-secondary)">Comprando o básico em inglês</div></div>
            <div class="card card-btn" onclick="expOpenStory(2)"><span class="tag tag-coral">Família</span><div style="font-size:15px;font-weight:500">Talking about my family</div><div style="font-size:13px;color:var(--color-text-secondary)">Uma apresentação simples</div></div>
            <div class="card card-btn" onclick="expGenStory()"><span class="tag tag-purple">+ Nova</span><div style="font-size:15px;font-weight:500">Gerar nova história</div><div style="font-size:13px;color:var(--color-text-secondary)">A IA cria para você</div></div>
          </div>
          <div id="exp-reading" style="display:none">
            <div id="exp-read-phase">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem">
                <p style="font-size:13px;color:var(--color-text-secondary)">Leia. Toque nas palavras destacadas para ver a tradução.</p>
                <button class="btn btn-sm" onclick="expHideText()">Pronto →</button>
              </div>
            ...
            <div id="exp-after-read" style="display:none">
              ...
              <button class="btn btn-primary" onclick="expStartChat()">Conversar (em inglês) →</button>
              <button class="btn btn-sm" style="margin-left:8px" onclick="expBackToStoryList()">Outra história</button>
            </div>
          </div>
          <div id="exp-chat" style="display:none">
            <button class="btn btn-sm" onclick="expBackToReading()" style="margin-bottom:1rem">← História</button>
            <div class="chat-wrap">
              ...
              <div class="chat-input-row">
                ...
                <textarea id="exp-input" rows="2" placeholder="Responda em inglês..."></textarea>
        ...
        <div id="exp-debate" style="display:none">
          ...
          <div id="exp-debate-setup">
            <p class="subtitle">A IA defende o lado oposto. Argumente em inglês.</p>
            ...
            <input id="exp-debate-topic" placeholder="Ou escreva seu tema (em inglês)..." style="width:100%;border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:10px 12px;font-size:15px;font-family:inherit;background:var(--color-background-primary);margin-bottom:8px;color:#1a1a18">
            <button class="btn btn-primary" onclick="expStartDebate()">Começar →</button>
          </div>
          <div id="exp-debate-chat" style="display:none">
            <div class="card" style="padding:.75rem 1rem;margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center">
              <span style="font-size:14px" id="exp-debate-label"></span><span class="tag tag-coral" style="margin:0">Debate</span>
            </div>
            <div class="chat-wrap">
              ...
              <div class="chat-input-row">
                ...
                <textarea id="exp-debate-input" rows="2" placeholder="Argumente em inglês..."></textarea>
        ...
        <div id="exp-topics" style="display:none">
          ...
          <div id="exp-topic-grid" class="topic-grid">
            <button class="topic-btn" onclick="expStartTopic('articles','Artigos (a/an/the)')"><div style="font-size:18px;margin-bottom:5px">A</div><div style="font-size:14px;font-weight:500">Artigos</div><div style="font-size:12px;color:var(--color-text-secondary)">a, an, the</div></button>
            <button class="topic-btn" onclick="expStartTopic('to_be','Verbo to be')"><div style="font-size:18px;margin-bottom:5px">↔</div><div style="font-size:14px;font-weight:500">Verbo to be</div><div style="font-size:12px;color:var(--color-text-secondary)">am, is, are</div></button>
            <button class="topic-btn" onclick="expStartTopic('present_simple','Presente simples')"><div style="font-size:18px;margin-bottom:5px">⏱</div><div style="font-size:14px;font-weight:500">Present simple</div><div style="font-size:12px;color:var(--color-text-secondary)">rotina e fatos</div></button>
            <button class="topic-btn" onclick="expStartTopic('past_simple','Passado simples')"><div style="font-size:18px;margin-bottom:5px">⟲</div><div style="font-size:14px;font-weight:500">Past simple</div><div style="font-size:12px;color:var(--color-text-secondary)">ontem, na semana passada</div></button>
            <button class="topic-btn" onclick="expStartTopic('prepositions','Preposições')"><div style="font-size:18px;margin-bottom:5px">↗</div><div style="font-size:14px;font-weight:500">Preposições</div><div style="font-size:12px;color:var(--color-text-secondary)">in, on, at...</div></button>
            <button class="topic-btn" onclick="expStartTopic('plurals','Plurais')"><div style="font-size:18px;margin-bottom:5px">✦</div><div style="font-size:14px;font-weight:500">Plurais</div><div style="font-size:12px;color:var(--color-text-secondary)">-s, -es, irregulares</div></button>
            <button class="topic-btn" onclick="expStartTopic('questions','Perguntas')"><div style="font-size:18px;margin-bottom:5px">❓</div><div style="font-size:14px;font-weight:500">Perguntas</div><div style="font-size:12px;color:var(--color-text-secondary)">do, does, did, wh-</div></button>
            <button class="topic-btn" onclick="expStartTopic('false_cognates','Falsos cognatos')"><div style="font-size:18px;margin-bottom:5px">⚠</div><div style="font-size:14px;font-weight:500">Falsos cognatos</div><div style="font-size:12px;color:var(--color-text-secondary)">pretend ≠ pretender</div></button>
          </div>
          <div id="exp-topic-chat" style="display:none">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
              <strong id="exp-topic-title"></strong>
              <button class="btn btn-sm" onclick="resetExpTopics()">← Tópicos</button>
            </div>
            <div class="chat-wrap">
              <div class="explore-messages" id="exp-topic-msgs"></div>
              <div class="explore-input-row">
                <input id="exp-topic-input" placeholder="Pergunte algo (em português)..." onkeydown="if(event.key==='Enter')expSendTopic()">
        ...
        <div id="exp-vocab" style="display:none">
          <button class="btn btn-sm" onclick="expBack()" style="margin-bottom:1rem">← Explorar</button>
          <h2 style="font-size:16px;font-weight:500;margin-bottom:1rem">Vocabulário salvo</h2>
